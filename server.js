              ...
              lab.attached_files_info.forEach(fileInfo => {
                  if (fileInfo.originalname) {
                      try {
                          ...
                          if (oldName !== fileInfo.originalname) {
                              fixedCount++;
                              console.log(`✅ Исправлено: "${oldName}" -> "${fileInfo.originalname}"`);
                          }
                      } catch (e) {
                          console.log('❌ Ошибка исправления:', fileInfo.originalname);
                      }
                  }
              });
          }
                  ...
                  submission.student_files_info.forEach(fileInfo => {
                      if (fileInfo.originalname) {
                          try {
                              ...
                              if (oldName !== fileInfo.originalname) {
                                  fixedCount++;
                                  console.log(`✅ Исправлено студенческое: "${oldName}" -> "${fileInfo.originalname}"`);
                              }
                          } catch (e) {
                              console.log('❌ Ошибка исправления студенческого файла:', fileInfo.originalname);
                          }
                      }
                  });
              }
